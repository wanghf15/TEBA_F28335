...
void InitXintf(void)
{
	...
	//我已在SysCtrl.c中对时钟进行过集体启动或者禁止，这里就不在需要了。lxj
	//I have initial the clock at SysCtrl.c (备注：lxj)
  ...
  XintfRegs.XINTCNF2.bit.XTIMCLK = 0;    //1: XCLKOUT = 1/2 SYSCLKOUT =75M  0:150
   // XCLKOUT = XTIMCLK/2
  XintfRegs.XINTCNF2.bit.CLKMODE = 0;   //1: XCLKOUT = XTIMCLK/2 = 37.5M  0:XCLKOUT = XTIMCLK
   // Enable XCLKOUT
  XintfRegs.XINTCNF2.bit.CLKOFF = 0;
    ...
    // double all Zone read/write lead/active/trail timing
    XintfRegs.XTIMING6.bit.X2TIMING = 1;   //对 引导 激活 扫尾均乘以2  其中乘以2后的引导为2*原来的设置+1

    // Zone will sample XREADY signal
    XintfRegs.XTIMING6.bit.USEREADY = 0;   //0:不需要对XREADY进行采样 1：需要对XREADY进行采样
    XintfRegs.XTIMING6.bit.READYMODE = 1;  // sample synchronous

    // Size must be either:
    // 0,1 = x32 or
    // 1,1 = x16 other values are reserved
    XintfRegs.XTIMING6.bit.XSIZE = 3;     //16位数据模式
